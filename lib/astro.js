...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "POPDEK-MD"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "POPDEK-MD") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "POPDEK-MD") => {
...
app.listen(randomPort, () =>
  console.log(
    "POPDEK-MD Server listening on http://localhost:" + randomPort + "/ "
  )
);
